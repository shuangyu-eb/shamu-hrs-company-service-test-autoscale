...
import shamu.company.timeoff.service.*;
...
public class TimeOffRequestServiceTests {
  ...
  @Mock
  private TimeOffRequestApprovalStatusService timeOffRequestApprovalStatusService;
  ...
  void findByRequestId() {
    ...
    Mockito.when(timeOffRequestRepository.findByRequestId(Mockito.any())).thenReturn(timeOffRequest);
  ...
  void getByApproverAndStatusFilteredByStartDay() {
    ...
    Mockito.when(timeOffRequestRepository.findByApproversAndTimeOffApprovalStatusFilteredByStartDay(Mockito.any(), Mockito.any(), Mockito.any(), Mockito.any())).thenReturn(Page.empty());

    Assertions.assertDoesNotThrow(() -> timeOffRequestService.getByApproverAndStatusFilteredByStartDay(Mockito.any(), Mockito.any(), Mockito.any(), Mockito.any()));
  }

  @Test
  void getPendingRequestsCount() {
    Mockito.when(timeOffRequestRepository.countByApproverIdAndTimeOffApprovalStatus(Mockito.any(), Mockito.any())).thenReturn(1);

    Assertions.assertDoesNotThrow(() -> timeOffRequestService.getPendingRequestsCount(new User("1")));
  }
  ...
  void whenNull_thenThrow() {
    ...
    Assertions.assertThrows(ResourceNotFoundException.class, () -> timeOffRequestService.getById(Mockito.any()));
  }

  @Test
  void whenNotNull_thenShouldSuccess() {
    Mockito.when(timeOffRequestRepository.findById(Mockito.any())).thenReturn(Optional.of(new TimeOffRequest()));
  ...
  void findRecentRequestByRequesterAndStatus() {
    Mockito.when(timeOffRequestRepository
        .findRecentApprovedRequestByRequesterUserId(Mockito.any(), Mockito.any(), Mockito.any())).thenReturn(new TimeOffRequest());
    Mockito.when(timeOffRequestMapper.convertToTimeOffRequestDto(Mockito.any())).thenReturn(new TimeOffRequestDto());

    Assertions.assertDoesNotThrow(() -> timeOffRequestService.findRecentRequestByRequesterAndStatus(Mockito.any(), Mockito.any(), Mockito.any()));
  }
  ...
  class findReviewedRequests {
    ...
    void init() {
      ...
      request = PageRequest.of(
          1, 10, Sort.by(TimeOffRequestService.SortFields.APPROVED_DATE.getValue()).descending());
    ...
    void whenStartDayIsNullAndEndDayIsNull_thenShouldSuccess() {
      ...
      Mockito.when(timeOffRequestRepository.findByRequesterUserIdFilteredByStartDay(Mockito.any(), Mockito.any(), Mockito.any(), Mockito.any())).thenReturn(timeOffRequests);
      Mockito.when(timeOffRequestMapper.convertToTimeOffRequestDto(Mockito.any())).thenReturn(new TimeOffRequestDto());

      Assertions.assertDoesNotThrow(() -> timeOffRequestService.findReviewedRequests("1", null, null, 1, 10));
    }
  ...
  class getOtherRequestsBy {
    ...
    void whenNotFirstDayAndManagerIsNull_thenShouldSuccess() {
      ...
      Mockito.when(timeOffRequestRepository
          .findByRequesterUserInAndTimeOffApprovalStatus(Mockito.any(), Mockito.any(), Mockito.any(), Mockito.any())).thenReturn(timeOffRequests);
  ...
  void updateTimeOffRequestStatus() {
    ...
    Mockito.when(timeOffRequestMapper.createFromTimeOffRequestUpdateDto(Mockito.any())).thenReturn(timeOffRequest);
    Mockito.when(timeOffRequestRepository.findById(Mockito.any())).thenReturn(Optional.of(timeOffRequest));
    Mockito.when(requestApprovalStatusRepository.findByName(Mockito.any())).thenReturn(timeOffRequestApprovalStatus);
    Mockito.when(timeOffRequestApprovalStatusService.findByName(Mockito.any())).thenReturn(timeOffRequestApprovalStatus);
    Mockito.when(timeOffRequestRepository.save(Mockito.any())).thenReturn(timeOffRequest);
    Mockito.when(timeOffRequestMapper.convertToTimeOffRequestDto(timeOffRequest)).thenReturn(timeOffRequestDto);

    Assertions.assertDoesNotThrow(() -> timeOffRequestService.updateTimeOffRequestStatus("1", updateDto, authUser));
    Assertions.assertEquals(timeOffRequestService.updateTimeOffRequestStatus("1", updateDto, authUser).getComment(), updateDto.getApproverComment());
    Assertions.assertEquals(timeOffRequestService.updateTimeOffRequestStatus("1", updateDto, authUser).getStatus(), updateDto.getStatus().name());
  }
  ...
  void setTimeOffDetailDtoIsCurrentUserPrivileged() {
    ...
    Assertions.assertDoesNotThrow(() ->
        Whitebox.invokeMethod(timeOffRequestService,"setTimeOffDetailDtoIsCurrentUserPrivileged", user, authUser, new TimeOffRequestDetailDto()));
  }
  ...
  void saveTimeOffRequest() {
    ...
    Mockito.when(timeOffPolicyService.getTimeOffPolicyById(Mockito.any())).thenReturn(timeOffPolicy);
    Mockito.when(timeOffRequestApprovalStatusService.findByName(Mockito.any())).thenReturn(timeOffRequestApprovalStatus);
    ...
    Mockito.when(timeOffRequestRepository.save( Mockito.any())).thenReturn(timeOffRequest);
    Mockito.when(timeOffRequestDateService.saveAllTimeOffRequestDates( Mockito.any())).thenReturn(timeOffRequestDates);

    Assertions.assertDoesNotThrow(() -> timeOffRequestService.saveTimeOffRequest(timeOffRequest, requestCreateDto, TimeOffApprovalStatus.AWAITING_REVIEW, approver));
  }

  @Test
  void findTimeOffRequests() {
    final User user = new User();
    final TimeOffApprovalStatus[] statuses= new TimeOffApprovalStatus[]{TimeOffApprovalStatus.AWAITING_REVIEW};
