...
public class JwtUtil {
  ...
  public static Jwt getJwt() {
    final String token = JWT.create().withIssuer(issuer).sign(Algorithm.HMAC256("secret"));
  ...
  public static String generateRsaToken() throws NoSuchAlgorithmException {
    ...
    final String userEmail = authenticationToken != null ? authenticationToken.getUserEmail() : "test@gmail.com";
    return JWT.create()
        .withIssuer(issuer)
        ...
        .withClaim(customNamespace + "email", userEmail)
  ...
  public static Jwt decode(final String token) {
    final Algorithm algorithm =
        Algorithm.RSA256((RSAPublicKey) PUBLIC_KEY, (RSAPrivateKey) PRIVATE_KEY);
    final JWTVerifier jwtVerifier = JWT.require(algorithm).withIssuer(issuer).build();
