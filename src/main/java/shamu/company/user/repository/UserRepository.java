...
public interface UserRepository extends JpaRepository<User, String>, UserCustomRepository {
  ...
  String ACTIVE_USER_QUERY = " and (u.deactivated_at is null " + "or (u.deactivated_at is not null "
      + "and u.deactivated_at > current_timestamp)) ";

  @Override
  Optional<User> findById(String id);

  @Query(value = "select * from users u" + " where u.id = unhex(?1) "
      + ACTIVE_USER_QUERY, nativeQuery = true)
  User findActiveUserById(String userId);

  @Query(value = "select * from users u"
      + " where u.user_personal_information_id=unhex(?1) ", nativeQuery = true)
  User findByUserPersonalInformationId(String personalInformationId);

  @Query(value = "select * from users u where"
      + " u.user_contact_information_id=unhex(?1) ", nativeQuery = true)
  ...
  @Query(value = "select u.* from users u "
      ...
      + "where uc.email_work = ?1 " + ACTIVE_USER_QUERY, nativeQuery = true)
  User findByEmailWork(String emailWork);

  @Query(value = "SELECT * FROM users u " + "WHERE u.manager_user_id = unhex(?1) "
      + ACTIVE_USER_QUERY, nativeQuery = true)
  List<User> findAllByManagerUserId(String id);

  List<User> findByManagerUser(User managerUser);

  @Query(value = "SELECT count(1) FROM users u" + " WHERE u.company_id = unhex(?1) "
      + ACTIVE_USER_QUERY, nativeQuery = true)
  Integer findExistingUserCountByCompanyId(String companyId);

  @Query(value = "SELECT * FROM users u" + " WHERE u.company_id = unhex(?1) "
      + ACTIVE_USER_QUERY, nativeQuery = true)
  ...
  @Query(value = "select * from users u" + " join jobs_users ju on u.id = ju.user_id"
      + " where u.company_id = unhex(?1)" + " and u.manager_user_id = unhex(?2) "
      + ACTIVE_USER_QUERY, nativeQuery = true)
  List<User> findSubordinatesByManagerUserId(String companyId, String userId);

  @Query(value = "select count(1) from users u " + "where u.manager_user_id = unhex(?1) "
      + "and u.company_id = unhex(?2) " + ACTIVE_USER_QUERY, nativeQuery = true)
